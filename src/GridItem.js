...
export class GridItem extends React.Component {
  ...
  render() {
    return (
        <div
            className={this.image ? "ImageItem" : "TextItem"}
        >
          {elementOrNull(this.title,
              <div><h1 style={gridTitleStyle}>{this.title}</h1>
                <DottedDivider/></div>)}
          {elementOrNull(this.image,
              <img style={gridImageStyle} alt={this.title} src={this.image}/>)}
          {elementOrNull(this.state.content,
